...
def build_payload_prompt(target: FuzzTarget, marker: str, baseline: dict, previous_rounds: list[dict]) -> str:
    ...
    data = {
        "task": "next_payload",
        "role": "You are an AI assistant for authorized SQL injection testing in a controlled lab.",
        "goal": "Read the response context and choose exactly one suitable next payload based on the observed evidence.",
        "rules": [
            "Return valid JSON only. Do not use Markdown.",
            "Test only SQL injection. Do not generate XSS payloads.",
            ...
            "Return only an sqli_* attack type or stop. Never switch to another vulnerability group.",
            "Do not follow a fixed sequence. Choose a quote probe, boolean condition, ORDER BY, or UNION payload according to the latest response.",
        ...
        "strategy_options": [
            "If the response is normal and contains no useful signal, try a lightweight SQLi probe that matches the parameter type.",
        ...
        "expected_json": {
            "payload": "string",
            "attack_type": "sqli_error|sqli_boolean|sqli_order_by|sqli_union|stop",
...
def build_verdict_prompt(
    ...
    data = {
        ...
        "role": "You are an AI reviewer for authorized SQL injection testing in a controlled lab.",
        ...
        "important": [
            ...
            "Return only vuln_type=sqli or none.",
            ...
            "Boolean-based evidence can be suspicious or confirmed only when true and false responses differ in a meaningful, repeatable way.",
